...
const I18N = {
  fr: {
    ...
    wa_text: "Posez vos questions directement depuis WhatsApp : scannez le QR code ou appuyez sur le bouton, puis envoyez « aide ». Messages vocaux 🎤 acceptés.",
    ...
    tg_text: "Ouvert à tous : scannez le QR code ou appuyez sur le bouton, puis envoyez /aide ou posez directement votre question, par écrit ou en message vocal 🎤.",
    ...
    examples: [
      ...
      "Génère une image d'un fablab futuriste au Tchad",
    ],
    ...
    mode_image: (p) => `Image générée par ${p} — cliquez pour la télécharger.`,
    drawing: "Création de l'image… (jusqu'à 30 s)",
    passage: "passage",
  ...
  en: {
    ...
    wa_text: "Ask your questions straight from WhatsApp: scan the QR code or tap the button, then send \"help\". Voice messages 🎤 welcome.",
    wa_button: "Open WhatsApp",
    wa_note: "Demo: Meta test number, only authorised numbers receive a reply.",
    tg_title: "Also on Telegram",
    tg_text: "Open to everyone: scan the QR code or tap the button, then send /help or simply ask your question, typed or as a voice message 🎤.",
    ...
    examples: [
      ...
      "Generate an image of a futuristic fablab in Chad",
    ],
    ...
    mode_image: (p) => `Image generated by ${p} — click to download.`,
    drawing: "Creating the image… (up to 30 s)",
    passage: "passage",
...
function renderAnswer(r) {
  ...
  const mode = r.mode === "image" ? (r.image_provider ? t("mode_image", r.image_provider === "gemini" ? "Gemini" : "Pollinations") : "") :
               r.mode === "extractive" ? t("mode_extractive") :
  ...
  const image = r.image && r.image.startsWith("data:image/")
    ? `<a class="gen-image" href="${r.image}" download="unipods-image"><img src="${r.image}" alt="${esc(r.answer)}" /></a>` : "";
  return `${speakBtn}${esc(r.answer).replace(/\n/g, "<br>")}${image}${sources ? `<div class="sources">${sources}</div>` : ""}
    ${r.warning ? `<div class="mode err">${esc(r.warning)}</div>` : ""}<div class="mode">${mode}</div>`;
}

const history = [];
// Aperçu côté client (le serveur décide) : message d'attente adapté aux demandes d'image.
const IMAGE_ASK = /^\s*(\/image|\/img|dessine|draw\b|.*\b(image|illustration|dessin|picture|affiche|poster|logo)\b)/i;

async function ask(question, { spoken = false } = {}) {
  history.push(question);
  stopSpeaking();
  addMsg(esc(question), "user");
  const pending = addMsg(esc(t(IMAGE_ASK.test(question) ? "drawing" : "searching")), "bot");
